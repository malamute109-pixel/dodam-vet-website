...
{
    if ( y < 42 || y > 85 ) return false;
    const double phase = (y-42.0)/(85.0-42.0);
    const double cx = 38.5 + 0.8*std::sin(phase*3.14159265358979323846);
    return std::abs(x-cx) <= 1.8;
}
static bool rightTrochleaHigh(double x, double y) { return leftTrochleaHigh(127.0-x,y); }

static bool forceLow(double x, double y)
{
    // Low-side approach pockets aligned to the four Fighting Spirit ramp directions.
    const bool p1main = x>=57 && x<=70 && y>=17 && y<=29;
    const bool p2main = x>=57 && x<=70 && y>=98 && y<=110;
    const bool patN = x>=57 && x<=70 && y>=47 && y<=60;
    const bool patS = x>=57 && x<=70 && y>=67 && y<=81;

    const bool lsu = x>=17 && x<=30 && y>=50 && y<=61;
    const bool lsl = x>=17 && x<=30 && y>=66 && y<=77;
    const bool rsu = x>=97 && x<=110 && y>=50 && y<=61;
    const bool rsl = x>=97 && x<=110 && y>=66 && y<=77;
    return p1main || p2main || patN || patS || lsu || lsl || rsu || rsl;
}

static bool ridgeIsolationLow(double x, double y)
{
    // Deliberate low-ground channels: the patella must never merge with either
    // unwalkable trochlear ridge through ISOM cliff expansion.
    if ( y < 42 || y > 85 ) return false;
    return (x>=43 && x<=50) || (x>=77 && x<=84);
}
...
// avoiding the rectangular v1.5 paste and custom-ramp donor artifacts.
                                       ...
                                       const Sc::Terrain_::Tiles & data, const Patch & p)
{
    ...
    for ( int x=0; x<p.w; ++x )
    {
        ...
        // Fighting Spirit's stock Jungle ramps live in the extended ramp groups.
        // Retain mid-height/flagged transition tiles as a compatibility fallback.
        if ( (group >= 1024 && size_t(tg.terrainType) == 1 && tg.groundHeight >= 16) ||
    ...
    for ( int x=0; x<p.w; ++x )
    {
        ...
        {
            if ( std::abs(ox)+std::abs(oy) > 4 ) continue;
...
{
    if ( argc < 4 ) {
        std::cerr << "usage: IsomTerrain.exe <jungle.cv5> <Fighting Spirit 1.3 MLC.scx> <output.scx>" << std::endl;
        return 2;
    }
    ...
    if ( rampSource.empty() || rampSource.getTileset() != TS ) {
        std::cerr << "Could not open Fighting Spirit Jungle ramp source" << std::endl;
        return 4;
    }
    ...
    // Repaint separation channels after all high-ground stamping. This guarantees
    // the center patella is disconnected from both side ridges; no ramp is copied
    // onto either ridge.
    for ( int ix=0; ix<=64; ++ix )
    {
        int y=42; if ( (ix+y)&1 ) ++y;
        for ( ; y<=85; y+=2 )
        {
            const double x = double(ix*2);
            if ( ridgeIsolationLow(x,double(y)) )
    ...
    // Stock Fighting Spirit Jungle ramps only. The four source windows are the
    // map's normal directional ramps, trimmed tightly to avoid rectangular donor scars.
    const std::vector<Patch> patches = {
        {28,8,12,12,    58,17, "P1_MAIN_FS"},
        {88,112,12,12,  58,99, "P2_MAIN_FS"},
        {88,112,12,12,  58,48, "PATELLA_NORTH_FS"},
        {28,8,12,12,    58,68, "PATELLA_SOUTH_FS"},
        {3,84,12,9,     18,51, "LEFT_SESAMOID_UPPER_FS"},
        {3,84,12,9,     18,68, "LEFT_SESAMOID_LOWER_FS"},
        {114,35,12,8,   98,51, "RIGHT_SESAMOID_UPPER_FS"},
        {114,35,12,8,   98,68, "RIGHT_SESAMOID_LOWER_FS"}
    };
    ...
    mapFile->setScenarioName(RawString("Patellar Luxation v1.8 FIGHTING SPIRIT RAMPS"));
    mapFile->setScenarioDescription(RawString("Terrain-only 1v1 test. Stock Fighting Spirit Jungle ramps; symmetric sesamoid access; isolated unwalkable trochlear ridges; center patella separated by low-ground channels. No resources."));
